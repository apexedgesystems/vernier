 ...
 *
 * Demonstrates the Phase 4 integration surface. The same binary stays
 * silent in production and emits a full report once the operator sets
...
static const mon::MonitorTag kDecoder{"decoder", 1};
static const mon::MonitorTag kIo{"io", 2};
...
static void processFrame(mon::Monitor& monitor, int frameIdx) {
  {
    VERNIER_MONITOR_SCOPE(monitor, "decode", kDecoder);
    std::this_thread::sleep_for(std::chrono::microseconds(800));
  }
  {
    VERNIER_MONITOR_SCOPE(monitor, "render", kDecoder);
    std::this_thread::sleep_for(std::chrono::microseconds(400));
  }
  VERNIER_MONITOR_INCREMENT(monitor, "frames", kDecoder);
  VERNIER_MONITOR_GAUGE(monitor, "queue_depth", kIo, frameIdx % 16);
}
...
int main() {
  ...
  monitor.setThreshold("decode", kDecoder.id, 5000);
